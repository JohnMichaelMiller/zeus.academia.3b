...
{
  [Fact]
  public async Task Handle_WhenCodeAllowed_PersistsDegree()
  {
    await using var dbContext = CreateInMemoryContext();
    var handler = new AddDegreeHandler(dbContext);

    var response = await handler.Handle(new AddDegreeCommand("bsc"), CancellationToken.None);

    Assert.Equal("BSC", response.Code);

    var persisted = await dbContext.Degrees.SingleAsync(x => x.Code == "BSC");
    Assert.Equal("BSC", persisted.Code);
  }
